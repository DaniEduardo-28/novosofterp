...
class ItemResource extends JsonResource
{
    ...
    public function toArray($request)
    {
        ...
        return [
            ...
            'brochure_pdf' => $this->brochure_pdf ? asset('storage/uploads/brochures/' . $this->brochure_pdf) : null,
            'analisis_pdf' => $this->analisis_pdf ? asset('storage/uploads/analisis/' . $this->analisis_pdf) : null,
            'bpa_pdf' => $this->bpa_pdf ? asset('storage/uploads/bpa/' . $this->bpa_pdf) : null,
            'registro_sanitario_pdf' => $this->registro_sanitario_pdf ? asset('storage/uploads/registros/' . $this->registro_sanitario_pdf) : null,
            ...
            'individual_items' => $this->sets->transform(function ($row, $key) {
                ...
                $brand = $row->individual_item->brand ? strtoupper($row->individual_item->brand->name) : '';
                $laboratory = $row->individual_item->laboratory ? strtoupper($row->individual_item->laboratory->name) : '';
                $stock = $row->individual_item->getStockByWarehouse();
                $currency = $row->individual_item->currency_type;
                if (empty($currency)) {
                ...
                return [
                    ...
                    'description' => $row->individual_item->description,
                    'internal_id' => $row->individual_item->internal_id,
                    'brand' => $brand,
                    'laboratory' => $laboratory,
                    'currency_type_id' => $currency->currency_type_id,
                    'currency_type_symbol' => $currency->symbol,
                    'purchase_unit_price' => $row->individual_item->purchase_unit_price,
                    'unit_type_text' => $row->individual_item->getUnitTypeText(),
                    'stock' => (int)$stock,
                    'warehouses' => collect($row->individual_item->warehouses)->transform(function ($row) {
                        return [
            ...
            'set_is_print' => (bool)$this->set_is_print,
