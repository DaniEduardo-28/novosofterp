...
class ItemSetController extends Controller
{
    ...
    {
        $individual_items = Item::whereWarehouse()
            ->whereTypeUser()
            ->whereNotIsSet()
            ->whereIsActive()
            ...
            ->transform(function ($row) {
                ...
                $brand = $row->brand ? strtoupper($row->brand->name) : '';
                $laboratory = $row->laboratory ? strtoupper($row->laboratory->name) : '';
                $stock = $row->getStockByWarehouse() ?? 0;
                ...
                return [
                    'id' => $row->id,
                    'full_description' => strtoupper($full_description) . ($brand ? ' - ' . $brand : '') . ($laboratory ? ' - ' . $laboratory : '') . ' | STOCK : ' . (int)$stock . ' ' . strtoupper($row->getUnitTypeText()),
                    ...
                    'unit_type_text' => $row->getUnitTypeText(),
    ...
    public function duplicate(Request $request)
    {
        ...
        $new->save();
        $new->sets()->delete();
        foreach ($obj->sets as $set) {

            $new->sets()->create([
                'individual_item_id' => $set['individual_item_id'],
