...
class SaleNoteController extends Controller
{
    ...
    public function tables()
    {
        ...
        $patients = Patients::select('id', DB::raw("CONCAT(name, ' ', COALESCE(last_name, '')) as description"))->get();
        $cycles = Cycles::select('id', 'name as description')->get();
    ...
    public function storeWithData($inputs)
    {
        ...
        try {
            ...
            foreach ($data['items'] as $row) {
                ...
                if (isset($sale_note_item->item->is_farmacia)) {
                    if (isset($sale_note_item->item->lots_farmacia)) {
                        $lotesSelecteds = $sale_note_item->item->lots_farmacia;
                        $document_factor = -1;
                        $quantity_saldo = $sale_note_item->quantity;
                        foreach ($lotesSelecteds as $item) {
                            $lot = ItemLotsGroup::query()->find($item->id);
                            if ($lot->quantity >= $quantity_saldo && $document_factor == -1) {
                                $lot->quantity = $lot->quantity + ($quantity_saldo * $document_factor);
                                $this->validateStockLotGroup($lot, $sale_note_item);
                                $lot->save();
                                break;
                            }else{
                                $quantity_saldo = $quantity_saldo - $lot->quantity;
                                $lot->quantity = $lot->quantity + ($lot->quantity * $document_factor);
                                $this->validateStockLotGroup($lot, $sale_note_item);
                                $lot->save();
                            }
                        }
...
*/
    ...
    public function phone(Request $request)
    {
        ...
        $client = Person::find($record->customer_id);
    ...
    public function email(Request $request)
    {
        ...
        foreach ($client->person_addresses as $mailxd) {
            if ($mailxd->email) {
                $email .= "; {$mailxd->email}";
            }
        }
