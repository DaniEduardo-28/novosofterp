...
class ItemController extends Controller
{
    ...
    public function getRecords(Request $request)
    {
        ...
        switch ($request->column) {
            ...
            case 'category':
                ...
                break;
            case 'act_principles':
                $records->whereHas('act_principles', function ($q) use ($request) {
                    $q->where('name', 'like', "%{$request->value}%");
                });
                break;
            case 'laboratory':
                $records->whereHas('laboratory', function ($q) use ($request) {
                    $q->where('name', 'like', "%{$request->value}%");
                });
                break;
            case 'pharma_action':
                $records->whereHas('pharma_action', function ($q) use ($request) {
                    $q->where('name', 'like', "%{$request->value}%");
                });
                break;
            case 'subgroup':
                $records->whereHas('subgroup', function ($q) use ($request) {
                    $q->where('name', 'like', "%{$request->value}%");
                });
                break;
            case 'patients':
                $records->whereHas('patients', function ($q) use ($request) {
                    $q->where('name', 'like', "%{$request->value}%");
                });
                break;
            case 'customer':
                $records->whereHas('customer', function ($q) use ($request) {
                    $q->where('name', 'like', "%{$request->value}%");
                });
                    break;
            case 'cycles':
                $records->whereHas('cycles', function ($q) use ($request) {
                    $q->where('name', 'like', "%{$request->value}%");
                });
                break;
    ...
    public function tables()
    {
        ...
        $brands = Brand::all();
        $active_principles = ActivePrinciples::all();
        $laboratory = Laboratory::all();
        $pharmacological_action = PharmacologicalAction::all();
        $subgroups = Subgroup::all();
        $patients = Patients::all();
        $customer = Person::where('type', 'customers')->get();
        $cycles = Cycles::all();
    ...
    public function store(ItemRequest $request)
    {
        ...
        if ($request->hasFile('brochure_pdf')) {
            $brochurePath = $request->file('brochure_pdf')->store($directory);
            $item->brochure_pdf = $brochurePath;
        }

        if ($request->hasFile('analisis_pdf')) {
            $analisisPath = $request->file('analisis_pdf')->store($directory);
            $item->analisis_pdf = $analisisPath;
        }

        if ($request->hasFile('bpa_pdf')) {
            $bpaPath = $request->file('bpa_pdf')->store($directory);
            $item->bpa_pdf = $bpaPath;
        }
    
        if ($request->hasFile('registro_sanitario_pdf')) {
            $registroPath = $request->file('registro_sanitario_pdf')->store($directory);
            $item->registro_sanitario_pdf = $registroPath;
        }
    ...
    public function printBarCodeX(Request $request)
    {
        ...
        // $stock = $item_warehouse->stock;
        $stock = $record->inventory_kardex->where('quantity', '>', '0')->last()->quantity;

        $width = ($format == 1) ? 81 : 104.1;
        $width = ($format == 4) ? 110 : $width;
        $height = ($format == 1) ? 26 : 24;
        $height = ($format == 4) ? 297 : $height;
    ...
    public function uploadAnalisis(Request $request)
    {
    ...
    if ($request->hasFile('file')) {
        ...
        $path = $file->store('uploads/analisis', 'public');
