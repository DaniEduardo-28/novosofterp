...
<body class="ticket">
    ...
    @endif
    <table class="full-width pt-0">
        <tr>
            <td class="text-center">
                <h4 style="font-size: 4px !important;">{{ $company->name }}</h4>
            </td>
        </tr>
        <tr>
            <td class="text-center">
                <h5 style="font-size: 4px !important;">{{ 'RUC ' . $company->number }}</h5>
            </td>
        </tr>
        <tr>
            <td class="text-center" style="text-transform: uppercase;">
                <h2 style="font-size: 3px !important;">
        ...
        <tr>
            <td class="text-center">
                <h2 style="font-size: 4px !important;">
    ...
    <table class="full-width mt-10 mb-10">
        ...
        <tbody>
            ...
            @if ($document->total_isc > 0)
                <tr>
                    <td colspan="4" class="text-right font-bold desc">ISC: {{ $document->currency_type->symbol }}
                    </td>
            ...
            @if (($document->retention || $document->detraction) && $document->total_pending_payment > 0)
                <tr>
                    <td colspan="4" class="text-right font-bold desc">M. PENDIENTE:
                        {{ $document->currency_type->symbol }}</td>
    ...
    <table class="full-width pt-0">
        ...
        <tr>
            @if ($row->code == '1000')
                <td class="desc">Son: <span class="font-bold">{{ $row->value }}
                        {{ $document->currency_type->description }}</span></td>
                @if (count((array) $document->legends) > 1)
        <tr>
            <td class="desc"><span class="font-bold">Leyendas</span></td>
        </tr>
        @endif
    @else
        <td class="desc">{{ $row->code }}: {{ $row->value }}</td>
        @endif
        </tr>
        @endforeach
        </tr>
        ...
        <tr>
            <td class="text-center"><img class="qr_code" src="data:image/png;base64, {{ $document->qr }}" />
            </td>
        </tr>
        <tr>
        ...
        @if (class_basename($document) === 'Document')
            @switch($document->document_type_id)
                @case('03')
                    <tr>
                        <td class="text-center desc pt-0">Representación impresa de la BOLETA DE VENTA ELECTRÓNICA.</td>
                    </tr>
                @break

                @case('01')
                    <tr>
                        <td class="text-center desc pt-0">Representación impresa de la FACTURA ELECTRÓNICA.</td>
                    </tr>
                @break

                @case('07')
                    <tr>
                        <td class="text-center desc pt-0">Representación impresa de la NOTA DE CRÉDITO ELECTRÓNICA.</td>
                    </tr>
                @break

                @case('08')
                    <tr>
                        <td class="text-center desc pt-0">Representación impresa de la NOTA DE DÉBITO ELECTRÓNICA.</td>
                    </tr>
                @break
