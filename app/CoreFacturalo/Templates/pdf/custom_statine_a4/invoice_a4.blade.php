        ...
        @if ($document->quotation_id)
            <tr>
                ...
                @isset($document->quotation->delivery_date)
                    ...
                    <td width="8px">:</td>
                    <td>{{ $document->date_of_issue->addDays($document->quotation->delivery_date)->format('d-m-Y') }}
                    </td>
                @endisset
            </tr>
    ...
    <table class="full-width mt-10 mb-10">
        <thead class="">
            <tr class="bg-grey">
                ...
                <th class="border-top-bottom text-center py-2" width="15%">LOTE</th>
        ...
        <tbody>
            @foreach ($document->items as $row)
                <tr>
                    ...
                    <td class="text-center align-top">
                        ...
                        @if ($itemLotGroup->date_of_due($row->item->IdLoteSelected) != null)
                            <p style="font-size: 9.5px"><span class="font-bold">N°.
                                </span>{{ $itemLotGroup->getLote($row->item->IdLoteSelected) }}</p>
                            <p style="font-size: 9.5px"><span class="font-bold">FV.
                                </span>{{ date('d/m/Y', strtotime($itemLotGroup->date_of_due($row->item->IdLoteSelected))) }}
                            </p>
                        @endif
                    </td>
                    </td>
            ...
            @if ($document->total_discount > 0 && $document->subtotal > 0)
                <tr>
                    <td colspan="7" class="text-right font-bold">SUBTOTAL: {{ $document->currency_type->symbol }}
                    </td>
            ...
            @if ($document->total_charge > 0)
                ...
                @else
                    <tr>
                        <td colspan="7" class="text-right font-bold">CARGOS: {{ $document->currency_type->symbol }}
                        </td>
                        <td class="text-right font-bold">{{ number_format($document->total_charge, 2) }}</td>
                    </tr>
                @endif
            ...
            @if ($document->perception)
                ...
                <tr>
                    <td colspan="7" class="text-right font-bold">PERCEPCIÓN: {{ $document->currency_type->symbol }}
                    </td>
            ...
            @if (($document->retention || $document->detraction) && $document->total_pending_payment > 0)
                <tr>
                    <td colspan="7" class="text-right font-bold">M. PENDIENTE:
                        {{ $document->currency_type->symbol }}</td>
            ...
            <td width="65%" style="text-align: top; vertical-align: top;">
                ...
                @if (in_array($document->document_type->id, ['01', '03']))
                    ...
                    @foreach ($accounts as $account)
                        @if ($account->currency_type_id == $document->currency_type->id)
                            <p>
