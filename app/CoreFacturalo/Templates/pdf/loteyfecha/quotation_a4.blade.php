...
<body>
        ...
        <tr>
            ...
            <td width="50%" style="text-align: left; padding-left: 20px;">
                <img width="140px" height="40px"
                    src="data:{{ mime_content_type($digemid) }};base64, {{ base64_encode(file_get_contents($digemid)) }}">
                <img width="40px" height="40px"
                    src="data:{{ mime_content_type($bpa) }};base64, {{ base64_encode(file_get_contents($bpa)) }}">
                <img width="90px" height="40px"
                    src="data:{{ mime_content_type($rnp) }};base64, {{ base64_encode(file_get_contents($rnp)) }}">
            </td>
            <td width="30%"></td>
        </tr>
    </table>
        ...
        <tbody>
            @foreach ($document->items as $row)
                @php
                    ...
                    $itemxd = \App\Models\Tenant\Item::find($row->item->id);
                    $image_url =
                        $itemxd->image !== 'imagen-no-disponible.jpg'
                            ? asset(
                                'storage' .
                                    DIRECTORY_SEPARATOR .
                                    'uploads' .
                ...
                <tr>
                    <td class="text-center align-top">
                        ...
                        @endif
                        @if ($image_url != 'imagen-no-disponible.jpg')
                            <div>
                                <img src="{{ $image_url }}" alt="{{ $company->name }}"
                                    style="opacity: 0.6; width: 90px; height: 80px; margin-left: 50px;">
                            </div>
                        @endif
                    </td>
                    <td class="text-center align-top">
                    ...
                    <td class="text-left">
                        @if ($row->item->name_product_pdf ?? false)
                            {!! $row->item->name_product_pdf . ' (' . $row->item->internal_id . ')' ?? '' !!}
                        @else
                            {!! $row->item->description . ' (' . $row->item->internal_id . ')' !!}
                        @endif
                        ...
                        @if ($itemxd->caracteristicas)
                            <p style="font-size: 9px">
                                <strong>Especificaciones Técnicas: </strong>
                                {!! $itemxd->caracteristicas !!}
                            </p>
                        @endif

                        @inject('itemLotGroup', 'App\Services\ItemLotsGroupService')
                        @php
                            if (is_array($row->item->lots_group)) {
                                $id = $row->item->lots_group;
                                $result = '';
                                foreach ($id as $item) {
                                    if ($item->date_of_due) {
                            ...
                            $fechaOriginal = $itemLotGroup->getLotDateOfDue($row->item->lots_group);
                            $fechaObjeto = new DateTime($fechaOriginal);
                            $fechaFormateada = $fechaObjeto->format('d/m/Y');
                        @endphp
                        <p style="font-size: 9px">
                            @if ($row->item->lots_group)
                                <strong>Lote: </strong>{{ strtoupper($itemLotGroup->getLote($row->item->lots_group)) }}
                            @endif
                            @if ($result !== '')
                                <br>
                ...
                <table class="full-width">
                    ...
                    @foreach ($accounts as $account)
                        <tr>
                            ...
                            <td class="border-box text-center" style="font-size: 9px">{{ $account->number }}</td>
                            <td class="border-box text-center" style="font-size: 9px">{{ $account->cci }}</td>
                        </tr>
                    @endforeach
                </table>
            </td>
            <td width="35%" class="text-center">
                @if ($document->seller->email == 'Contabilidad7mdp@gmail.com')
                    <img width="200px" height="100px"
                        src="data:{{ mime_content_type($firma2) }};base64, {{ base64_encode(file_get_contents($firma2)) }}">
                @else
    ...
    <table class="full-width border-bottom-left-right">
        <tr>
            <td>
                <strong>La empresa 7M DROGUERIAS PERUANAS S.A.C., no aceptará las Ordenes de compras por un monto menor
                    de S/
                    500.00 soles, de las cuales se reserva el derecho de cancelar cualquier OC.</strong>
            </td>
        </tr>
    </table>
</body>
