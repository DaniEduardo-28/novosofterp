...
class UserCommissionHelper
{
    ...
    public static function getUtilityRecords($records)
    {
        ...
        return $records->sum(function ($record) {
            ...
            return $record->items->sum(function ($item) use ($record) {
                ...
                $total_item_sale = self::calculateTotalCurrencyType($record, $item->total_value);
                $total_item = $total_item_sale;
    ...
    public static function getDataForReportCommission($user, $request)
    {
        ...
        $acum_sales_document = $documents->sum(function ($document) {
            return $document->getTotalByDocumentType2();
        });
    ...
    public static function getTotalCommision($records)
    {
        ...
        return $records->sum(function ($record) {
            $seller_id = $record->seller->id;
            $amounts = UserCommission::where('user_id', $seller_id)->get();
            $amount = 1.5;
            if (count($amounts) > 0) {
                $amount = $amounts[0]->amount;
            }
            return $record->items->sum(function ($item) use ($record, $amount) {
                $total_commision = $item->total_value * ($amount / 100);
