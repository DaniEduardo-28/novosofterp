<template>
    <div class="card mb-0 pt-2 pt-md-0">
                ...
                <form autocomplete="off" @submit.prevent="submit">
                                ...
                                <table class="table table-responsive table-bordered">
                                    <thead>
                                        <tr width="100%">
                                            ...
                                            <th :width="form.additional_data.length > 0 ? '20%' : '5%'"><a href="#"
                                            ...
                                            <td>
                                                <div class="form-group mb-2 mr-2">
                                                    ...
                                                    <template v-if="errors[`additional_data.${index}.title`]">
                                                        <div class="form-group"
                                                            :class="{ 'has-danger': errors[`additional_data.${index}.title`] }">
                                                            <small class="form-control-feedback"
                                                                v-text="errors[`additional_data.${index}.title`][0]"></small>
                                                        </div>
                                                    </template>
                                                </div>
                                            </td>
                                            <td>
                                                <div class="form-group mb-2 mr-2">
                                                    ...
                                                    <template v-if="errors[`additional_data.${index}.description`]">
                                                        <div class="form-group"
                                                            :class="{ 'has-danger': errors[`additional_data.${index}.description`] }">
                                                            <small class="form-control-feedback"
                                                                v-text="errors[`additional_data.${index}.description`][0]"></small>
                                                        </div>
                                                    </template>
                                                </div>
                                            </td>
                                            ...
                                            <tr v-for="(row, index) in form.items">
                                                ...
                                                <td class="text-center">
                                                    ...
                                                    <template v-if="row.id">
                                                        <button type="button"
                                                            class="btn waves-effect waves-light btn-xs btn-danger"
                                                            @click.prevent="clickDeleteONItem(row.id, index)">x</button>
                                                    </template>
                                                    <template v-else-if="row.record_id">
                                                        <button type="button"
                                                            class="btn waves-effect waves-light btn-xs btn-danger"
                                                            @click.prevent="clickDeleteONItem(row.record_id, index)">x</button>
                                                    </template>
                                                    <template v-else>
                                                        <button type="button"
                                                            class="btn waves-effect waves-light btn-xs btn-danger"
                    ...
                    <div class="form-actions text-right mt-4">
                        ...
                        <el-button class="submit" type="primary" native-type="submit" :loading="loading_submit"
                            v-if="form.items.length > 0">Guardar cambios</el-button>
                    </div>
                </form>
            </div>
        </div>

        <order-note-form-item :configuration="configuration" :currency-type-id-active="form.currency_type_id"
            :exchange-rate-sale="form.exchange_rate_sale" :showDialog.sync="showDialogAddItem" :typeUser="typeUser"
        ...
        <order-note-options :type="type" :showDialog.sync="showDialogOptions" :recordId="orderNoteNewId"
            :showGenerate="false" :typeUser="typeUser" :showClose="false"></order-note-options>
    </div>
</template>
