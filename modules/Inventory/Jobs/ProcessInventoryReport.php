    ...
    public function getRecordsTranform($warehouse_id, $filter)
    {
        ...
        $records->chunk(1000, function ($items) use (&$data) {
            foreach ($items as $row) {
                $item = $row->item;
                $itemdbxd = Item::find($row->id);

                $stock_unit = $row->stock;
                $stock_caja = 0;
                $precio_caja = 0;
                $factor = 1;
                ...
                if ($itemdbxd !== null) {
                    ...
                    $item_unit_types = $itemdbxd->getApiDataItemUnitTypes();

                    if ($item_unit_types->count() > 0) {
                        $firstItem = $item_unit_types->first();
                        $precio_caja = $firstItem['price2'] ?? 0;
                    }

                    $hay_caja = false;
                    if ($item_unit_types->count() > 0 && $stock_unit > 0) {
                        $firstItem = $item_unit_types->first();
                        $factor = $firstItem['quantity_unit'] ?? 1;
                        if ($stock_unit > $factor && strtoupper($firstItem['description']) == 'CAJA') {
                            $stock_caja = intdiv($stock_unit, $factor);
                            $hay_caja = true;
                        }
                    }

                    if ($hay_caja) {
                        $stock_unit = $stock_unit % $factor;
                    }
                }
