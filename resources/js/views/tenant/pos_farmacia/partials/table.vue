<template>
  <div>
    ...
    <Keypress
      key-event="keyup"
      :key-code="113"
      @success="openTableListPrices113"
    />
    ...
    >
      ...
      <el-table-column property="description" label="Nombre" width="170">
      </el-table-column>
      <el-table-column property="principio_activo" label="P.A." width="120">
      </el-table-column>
      <!-- <el-table-column property="lots" label="Lote" width="100">
      </el-table-column> -->
      <el-table-column label="Lotes">
        <template slot-scope="{ row }">
          <template v-if="row.lots_farmacia.length > 0">
            <el-popover
              placement="top"
              title="Lotes"
              width="450"
              trigger="click"
            >
              <el-table v-if="row.lots_farmacia" :data="row.lots_farmacia">
                <el-table-column width="150" label="Código">
                  <template slot-scope="scope">
                    <span v-html="scope.row.code_string"></span>
                    <!-- Usamos v-html para mostrar el código -->
                  </template>
                </el-table-column>
                <el-table-column width="150" label="F.V.">
                  <template slot-scope="scope">
                    <span v-html="scope.row.date_of_due"></span>
                    <!-- Usamos v-html para mostrar la fecha de vencimiento -->
                  </template>
                </el-table-column>
                <el-table-column width="75" label="Cantidad">
                  <template slot-scope="scope">
                    <span>{{ scope.row.quantity }}</span>
                    <!-- Mostrar cantidad sin formato especial -->
                  </template>
                </el-table-column>
              </el-table>
              <button
      ...
      <el-table-column property="laboratorio" label="LAB." width="150">
      </el-table-column>
      <el-table-column property="accion_farmacologica" label="A.F." width="150">
      </el-table-column>
      <!-- <el-table-column property="currency_type_id" label="Moneda" width="80">
                </el-table-column> -->
      <el-table-column label="Precio" width="70">
        <template slot-scope="{ row }">
      ...
      <el-table-column label="Stock">
        <template slot-scope="{ row }">
          ...
          <div v-else>
            ...
            <template
              v-else-if="typeUser != 'seller' && row.unit_type_id != 'ZZ'"
            >
              <button
                type="button"
        ...
        <template slot-scope="{ row }">
          <template v-if="row.unit_type.length > 0">
            <el-popover
              ...
              trigger="click"
            >
              <el-table v-if="row.unit_type" :data="row.unit_type">
        ...
        <template slot-scope="{ row }">
          <template>
            <button
              ...
              @click.prevent="seleccionarItem(row)"
            >
