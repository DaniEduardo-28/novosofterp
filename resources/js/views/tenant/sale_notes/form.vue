<template>
    <div class="card mb-0 pt-2 pt-md-0">
                ...
                <form autocomplete="off" @submit.prevent="submit">
                            ...
                            <div class="col-lg-2 col-md-2">
                                <div class="form-group">
                                    ...
                                    <el-select v-model="form.type_period" clearable>
                                        <el-option v-for="option in type_periods" :key="option.id" :value="option.id"
                                            :label="option.description"></el-option>
                                    </el-select>
                            ...
                            <div v-if="config.active_allowance_charge && form.total > 0" class="col-lg-2 col-md-2">
                                <div class="form-group">
                                    ...
                                    <el-input-number v-model="config.percentage_allowance_charge" :min="0"
                                        controls-position="right" size="mini" @change="calculateTotal">
                                    </el-input-number>
                                </div>
                            </div>
                                            ...
                                            <tr v-for="(row, index) in form.items" :key="index">
                                                ...
                                                <td class="text-center">
                                                    ...
                                                    <button class="btn waves-effect waves-light btn-xs btn-info"
                                                        type="button" @click="clickEdiItem(row, index)">
                                ...
                                <div class="form-group">
                                    <div v-if="isCustomerSelected">
                                        <el-popover placement="top-start" :open-delay="1000" width="145" trigger="hover"
                                            content="Presiona F2">
                                            <el-button slot="reference" type="button"
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <el-input-number v-model="total_global_charge"
                                                            ...
                                                            :min="0" class="input-custom ml-2" controls-position="right"
                    ...
                    <div class="form-actions text-right mt-4">
                        ...
                        <el-popover placement="top-start" width="145" trigger="hover" content="Presiona ALT + G">
                            <el-button slot="reference" class="submit" type="primary" native-type="submit"
                                :loading="loading_submit" v-if="form.items.length > 0">
                                Generar
                            </el-button>
                        </el-popover>
                    </div>
        ...
        <sale-notes-form-item :typeUser="typeUser" :showDialog.sync="showDialogAddItem"
            ...
            :currency-types="currency_types" :show-option-change-currency="true" @add="addRow"></sale-notes-form-item>

        <person-form :showDialog.sync="showDialogNewPerson" type="customers" :external="true"
            :input_person="input_person" :document_type_id=form.document_type_id></person-form>
...
export default {
    ...
    computed: {
        ...
        isCustomerSelected() {
            return this.form.customer_id !== null;
        },
        showLoadVoucher() {
            ...
            if (this.id) {
                ...
                await this.$http.get(`/${this.resource}/record2/${this.id}`)
                    .then(response => {
                        ...
                        console.log('EDITANDO FORM:', this.form)
                        if (this.form.patients_id) {
    const exists = this.patients.find(p => p.id === this.form.patients_id);
    if (!exists) {
        this.patients.push({
            id: this.form.patients_id,
            description: this.form.patients, // o nombre del paciente si viene en otro campo
        });
    }
}

// Asegurarse de que el ciclo actual esté en la lista
if (this.form.cycles_id) {
    const exists = this.cycles.find(c => c.id === this.form.cycles_id);
    if (!exists) {
        this.cycles.push({
            id: this.form.cycles_id,
            description: this.form.cycles, // igual, si viene con otro campo
        });
    }
}
                        this.setDataUpdate()
