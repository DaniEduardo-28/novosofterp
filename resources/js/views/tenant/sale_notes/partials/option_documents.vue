<template>
    <div>
        <el-dialog :title="titleDialog" :visible="show" @open="create" width="50%" :close-on-click-modal="false"
            ...
            <div class="row">
                ...
                <div class="col-lg-12">
                    <div class="form-group">
                        ...
                        <el-input type="textarea" autosize v-model="document.additional_information">
                        </el-input>
                    </div>
                </div>
                ...
                <div class="col-lg-4">
                    <div class="form-group" :class="{ 'has-danger': errors.payment_condition_id }">
                        ...
                        <el-select v-model="document.payment_condition_id" @change="changePaymentCondition"
                            popper-class="el-select-document_type" dusk="document_type_id" style="max-width: 200px;">
                ...
                <div class="col-lg-8 mt-3">
                    <div class="form-group" :class="{ 'has-danger': errors.dipatch_id }">
                        <!-- <label class="control-label">Tipo comprobante</label> -->
                        <el-checkbox v-model="generate_dispatch">Generar Guía Remisión</el-checkbox>
                        <el-select v-model="dispatch_id" popper-class="el-select-document_type" filterable
                            class="border-left rounded-left border-info" v-if="generate_dispatch">
                            <el-option v-for="option in dispatches" :key="option.id" :value="option.id"
                                :label="option.number_full"></el-option>
                        </el-select>
                        <small class="form-control-feedback" v-if="errors.dipatch_id"
                    ...
                    <table v-if="document.fee.length > 0" width="100%">
                        <thead>
                            <tr>
                                ...
                                <th>Monto</th>
                                <th style="width: 30px">
                                    <a style="font-size:18px" href="#" @click.prevent="clickAddFee"
                                ...
                                <td v-if="document.fee.length > 0">
                                    <div class="form-group mb-2 mr-2">
                                        <el-date-picker v-model="row.date" type="date" value-format="yyyy-MM-dd"
                                            format="dd/MM/yyyy" @change="changeDatePaymentCondition(index)"
                                            :clearable="false"></el-date-picker>
                                    </div>
                                </td>
                                <td v-if="document.fee.length > 0">
            ...
            <span slot="footer" class="dialog-footer">
                ...
                <el-button class="submit" type="primary" @click="submit" :loading="loading_submit"
                    v-if="flag_generate">Generar</el-button>
            </span>

            <document-options :showDialog.sync="showDialogDocumentOptions" :recordId="documentNewId"
                :generatDispatch="generate_dispatch" :dispatchId="dispatch_id" :isContingency="false"
