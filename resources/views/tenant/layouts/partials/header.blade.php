<header class="header">
    <div class="logo-container">
        ...
        @if ($pagosDB)
            @if ($pagosDB->date_of_payment <= date('Y-m-d'))
                @php
                ...
                @endphp
                <div class="ml-3 mr-3 text-white bg-danger pl-3 pr-3">
                    <strong>
                        <h4>{{ strtoupper('Tu recibo VENCIÓ, SE BLOQUEARÁ EN ' . $diasVencer . ($diasVencer == 1 ? ' DÍA.' : ' DÍAS.')) }}
                        </h4>
                    </strong>
                </div>
            @else
                @php
                    $fActual = new DateTime();
                    $diasVencer = $fActual->diff($pagosDB->date_of_payment)->days;
                @endphp
                @if ($diasVencer < 1)
                    <div class="ml-3 mr-3 text-white bg-danger pl-3 pr-3">
                        <strong>
                            <h4>{{ strtoupper('Tu recibo Vence el dÍa de hoy.') }}
                            </h4>
                        </strong>
                    </div>
                @else
                    <div class="ml-3 mr-3 text-white bg-warning pl-3 pr-3">
                        <strong>
                            <h4>{{ strtoupper('Tu recibo VencerÁ EN ' . $diasVencer . ($diasVencer == 1 ? ' DÍA.' : ' DÍAS.')) }}
                            </h4>
                        </strong>
                    </div>
                @endif
